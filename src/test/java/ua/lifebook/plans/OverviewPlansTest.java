...
public class OverviewPlansTest {
    final Map<Integer, PlansByDay> emptyPlans = new HashMap<>();
    ...
    public void allWeeksHave7Days() {
        ...
        int week = 1;
        for (List<PlansByDay> plans : overviewPlans.getData()) {
    ...
    public void checkDaysNumbers() {
        ...
        int day = 27;
        for (int i = 0; i < 5; i += 1) {
            final List<PlansByDay> row = overviewPlans.getData().get(i);
            for (int j = 0; j < 7; j += 1) {
                final PlansByDay plansByDay = row.get(j);
                assertThat("Week #" + (i + 1) + " contains wrong day", day, is(plansByDay.getDayOfMonth()));
                final LocalDate localDate = plansByDay.getDayFully().toLocalDate();
